...
class AplicativoPedidosMagico:
    def __init__(self, root):
        ...
        if GEMINI_DISPONIVEL and self.gemini_api_key:
            try:
    ...
    def carregar_config(self):
        if os.path.exists(CONFIG_PATH):
            try:
                ...
                self.gemini_api_key = dados.get("gemini_api_key", "")
            except Exception:
                pass 
    ...
    def _executar_atualizacao(self, url_download):
        try:
            self.set_status("A baixar atualização do GitHub...")
    ...
    def verificar_e_instalar_ollama_automatico(self):
        ...
        if not instalado:
            ...
            if resposta:
                self.set_status("A descarregar o instalador do Ollama...")
                threading.Thread(target=self._executar_download_instalacao_ollama, daemon=True).start()
        else:
            self.set_status("Ollama detectado e pronto para uso local.")
    ...
    def _executar_download_instalacao_ollama(self):
        try:
            ...
            self.root.after(0, lambda: self.set_status("A baixar o Ollama..."))
            urllib.request.urlretrieve(url_installer, caminho_temp)
            
            self.root.after(0, lambda: self.set_status("A executar o instalador do Ollama..."))
            subprocess.run([caminho_temp, "/VERYSILENT", "/SUPPRESSMSGBOXES", "/NORESTART"], check=True)
            
            self.root.after(0, lambda: self.set_status("Ollama instalado! A baixar o modelo 'llama3'..."))
            subprocess.run(["ollama", "pull", "llama3"], check=True)
            
            self.root.after(0, lambda: messagebox.showinfo("Sucesso", "Ollama e o modelo de IA foram instalados com sucesso!"))
            self.root.after(0, lambda: self.set_status("IA Local pronta para uso."))
        except Exception as e:
            err = str(e)
            self.root.after(0, lambda: messagebox.showerror("Erro na instalação", f"Não foi possível concluir a instalação:\n{err}"))
            self.root.after(0, lambda: self.set_status("Falha na instalação automática do Ollama."))
    ...
    def criar_interface(self):
        ...
        menu_arquivo.add_separator()
        menu_arquivo.add_command(label="🖼️ Ler Imagem com IA...", command=self.ler_imagem_ocr)
        ...
        menu_config.add_command(label="🔑 Configurar Chave da API (Gemini)...", command=self.configurar_api_gemini)
        menu_config.add_separator()
        ...
        self.lbl_subtitulo = tk.Label(self.header_frame, text=f"Versão {__version__} | Atualização Automática via GitHub e IA Local", font=("Segoe UI", 9), bg="#1E293B", fg="#94A3B8")
        ...
        tk.Button(frame_ocr_rapido, text="🖼️ Ler Imagem (IA)", command=self.ler_imagem_ocr, bg="#7C3AED", fg="white", font=("Segoe UI", 9, "bold"), bd=0, cursor="hand2", padx=8, pady=4).pack(side="left", padx=(0, 4))
        tk.Button(frame_ocr_rapido, text="📋🖼️ Colar (IA)", command=self.colar_imagem_ocr, bg="#7C3AED", fg="white", font=("Segoe UI", 9, "bold"), bd=0, cursor="hand2", padx=8, pady=4).pack(side="left", padx=(0, 4))
        tk.Button(frame_ocr_rapido, text="💻 Ler Offline", command=self.ler_imagem_ocr_offline, bg="#475569", fg="white", font=("Segoe UI", 9, "bold"), bd=0, cursor="hand2", padx=8, pady=4).pack(side="left", padx=(0, 4))
        tk.Button(frame_ocr_rapido, text="📋💻 Colar Offline", command=self.colar_imagem_ocr_offline, bg="#475569", fg="white", font=("Segoe UI", 9, "bold"), bd=0, cursor="hand2", padx=8, pady=4).pack(side="left")
        ...
        self.btn_gerar_ia = tk.Button(frame_botoes_topo, text="🤖 ORGANIZAR COM GEMINI", command=self.processar_texto_com_ia_thread, bg="#7C3AED", fg="white", font=("Segoe UI", 10, "bold"), activebackground="#6D28D9", activeforeground="white", bd=0, cursor="hand2", padx=8, pady=8)
        self.btn_gerar_ia.pack(side="left", padx=(0, 5), ipadx=5, fill="x", expand=True)
        ...
        self.status_var = tk.StringVar(value="Pronto para processamento Turbo.")
        self.status_bar = tk.Label(self.root, textvariable=self.status_var, bd=1, relief="sunken", anchor="w", font=("Segoe UI", 8), bg="#E2E8F0", fg="#334155")
        self.status_bar.pack(fill="x", side="bottom")

        footer_frame = tk.Frame(self.root, bg="#E2E8F0", pady=8)
        footer_frame.pack(fill="x", side="bottom")
        lbl_assinatura = tk.Label(footer_frame, text="Desenvolvido por Douglas Oliveira  |  Contato: getyourwish10@gmail.com", font=("Segoe UI", 9, "bold"), bg="#E2E8F0", fg="#475569")
        lbl_assinatura.pack()
    ...
    def _configurar_atalhos(self):
        ...
        self.root.bind("<Control-Shift-V>", lambda e: self.colar_imagem_ocr())
        self.root.bind("<Control-Shift-v>", lambda e: self.colar_imagem_ocr())
    ...
    def alternar_tema(self):
        ...
        self.status_bar.configure(bg=p["border"], fg=p["text"])
    ...
    def processar_texto(self):
        ...
        self.set_status(f"{len(pedidos_processados)} pedido(s) organizado(s). {len(linhas_nao_reconhecidas)} linha(s) não reconhecida(s).")

    # --- PROCESSAMENTO COM IA LOCAL (OLLAMA) ---
    def processar_texto_com_ia_local_thread(self):
        ...
        self.set_status("A IA Local (Ollama) está a processar os dados...")
    ...
    def _processar_texto_com_ia_local_exec(self, texto_bruto):
        try:
            ...
            def atualizar_interface_sucesso():
                ...
                self._redesenhar_tabela()
                self._atualizar_avisos()
                self.set_status(f"Sucesso! {len(pedidos_processados)} pedido(s) organizados por IA Local.")
            ...
            def mostrar_erro():
                ...
                self.set_status("Erro na IA Local.")
            self.root.after(0, mostrar_erro)

    # --- GEMINI API COM RETRY ---
    def _chamar_gemini_com_retry(self, contents):
        ...
        while tentativa < max_tentativas:
            try:
                ...
                erro_str = str(ex)
                if "PerDay" in erro_str or "GenerateRequestsPerDay" in erro_str or "EXHAUSTED" in erro_str:
                    raise Exception(
                        "🛑 LIMITE DIÁRIO DE USO DO GEMINI ATINGIDO!\n\n"
                        "💡 Dica: Use o botão verde '🏠 ORGANIZAR COM IA LOCAL (Ollama)' para processar tudo sem limites!"
                    )
    ...
    def processar_texto_com_ia_thread(self):
        texto_bruto = self.text_area.get("1.0", tk.END).strip()
        if not texto_bruto:
            messagebox.showwarning("Aviso", "A caixa de texto está vazia!")
            return
        if not self._dependencias_ocr_ok():
            return

        self.set_status("A Inteligência Artificial Gemini está a organizar os pedidos...")
        threading.Thread(target=self._processar_texto_com_ia_exec, args=(texto_bruto,), daemon=True).start()

    def _processar_texto_com_ia_exec(self, texto_bruto):
        try:
            prompt = f"""
            És um assistente especialista em organizar listas de uniformes para gráficas.
            Analisa o texto desorganizado abaixo e converte-o numa lista estruturada.
            ...
            Regras estritas:
            - Se a linha contiver a palavra "CONJUNTO", o tamanho do calção deve ser igual ao tamanho da camisa.
            - Se o cliente usar o número da camisa como "π" ou "PI", identifique-o na coluna de número.
            - Ignora numerações ordinais no início da linha.
            - Se faltar dado, preenche com "SEM NOME", "SEM TAMANHO", "SEM NÚMERO", "SEM CALÇÃO".
            - Retorna apenas as linhas no formato especificado, sem formatação markdown.

            Texto bruto:
            {texto_bruto}
            """

            resposta_texto = self._chamar_gemini_com_retry([prompt])
            resposta_texto = resposta_texto.replace("```csv", "").replace("```", "").strip()
            linhas_resposta = resposta_texto.splitlines()
            pedidos_processados = []
            ...
            if not pedidos_processados:
                self.root.after(0, lambda: messagebox.showwarning("Aviso", "A IA não conseguiu estruturar os dados."))
                return
            ...
            def atualizar_interface_sucesso():
                ...
                self._redesenhar_tabela()
                self._atualizar_avisos()
                self.set_status(f"Sucesso! {len(pedidos_processados)} pedido(s) organizados por Gemini.")

            self.root.after(0, atualizar_interface_sucesso)

        except Exception as e:
            erro_msg = str(e)
            self.root.after(0, lambda: messagebox.showerror("Aviso sobre os Limites da IA", erro_msg))

    # --- OCR / IMAGEM ---
    def _dependencias_ocr_ok(self):
        if not PIL_DISPONIVEL or not GEMINI_DISPONIVEL:
            messagebox.showerror("Recurso indisponível", "Instale Pillow e google-genai.")
            return False
        if not self.gemini_api_key:
            self.configurar_api_gemini()
            if not self.gemini_api_key:
                return False
        return True

    def configurar_api_gemini(self):
        chave = simpledialog.askstring("Configurar API Gemini", "Cole sua chave da Gemini API:", show='*', parent=self.root)
        if chave:
            self.gemini_api_key = chave.strip()
            try:
                self.client_gemini = genai.Client(api_key=self.gemini_api_key)
                self.salvar_config()
                self.set_status("Chave API configurada com sucesso.")
            except Exception as e:
                messagebox.showerror("Erro", f"Chave inválida:\n{e}")

    def ler_imagem_ocr(self):
        if not self._dependencias_ocr_ok():
            return
        ...
        try:
            ...
            self._rodar_ocr_com_revisao_thread(imagem, os.path.basename(caminho))
        except Exception as e:
            messagebox.showerror("Erro", f"Erro ao abrir imagem:\n{e}")

    def colar_imagem_ocr(self):
        if not self._dependencias_ocr_ok():
            return
        try:
        ...
        if imagem is None:
            ...
            return
        self._rodar_ocr_com_revisao_thread(imagem, "área de transferência")

    def _rodar_ocr_com_revisao_thread(self, imagem, origem):
        self.set_status(f"A ler imagem ({origem})...")
        threading.Thread(target=self._rodar_ocr_exec, args=(imagem,), daemon=True).start()

    def _rodar_ocr_exec(self, imagem):
        try:
            img_rgb = imagem.convert('RGB')
            prompt = "Transcreve absolutamente todo o texto contido nesta imagem, mantendo a estrutura exata das linhas."
            texto = self._chamar_gemini_com_retry([img_rgb, prompt])
            if texto:
                self.root.after(0, lambda: self._mostrar_janela_revisao_ocr(texto))
        except Exception as e:
            err = str(e)
            self.root.after(0, lambda: messagebox.showerror("Erro OCR", err))
    ...
    def colar_imagem_ocr_offline(self):
        if not PYTESSERACT_DISPONIVEL:
            self._mostrar_aviso_instalacao_tesseract()
            return
        try:
            ...
            imagem = None
        if imagem is None:
            messagebox.showinfo("Aviso", "Nenhuma imagem na área de transferência.")
            return
        try:
    ...
    def _mostrar_janela_revisao_ocr(self, texto):
        ...
        janela.title("Revisar texto OCR")
